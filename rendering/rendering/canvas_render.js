class CanvasRenderer {
    ...
    #animate() {
        ...
        this.canvas_objs.forEach((obj) => { if (obj.update) obj.update(obj); });
        this.canvas.renderAll();
        if (this.#running) setTimeout(this.#animate, this.#frame_time);
    }

    obj_from_img(path, position, options = {
        selectacle: false,
        name: "",
        update: null
    }) {
    ...
    zoom(x, y, scale) {
        ...
        this.canvas.zoomToPoint(new fabric.Point(pos.x, pos.y), this.canvas.getZoom() * scale);
    }
